...
class BP_Members_Invitations_List_Table extends WP_Users_List_Table {
	...
	public function views() {
		...
		<h2 class="screen-reader-text"><?php
			...
			_e( 'Filter invitations list', 'buddypress' );
		?></h2>
		<ul class="subsubsub">
			<li class="all">
	...
	public function no_items() {
		...
		if ( bp_get_members_invitations_allowed() ) {
			esc_html_e( 'No pending invitations found.', 'buddypress' );
		} else {
	...
	public function column_invitee_email( $invite = null ) {
		...
		if ( ! $invite->invite_sent || $invite->accepted ) {
			...
			$actions['delete'] = sprintf( '<a href="%1$s" class="delete">%2$s</a>', esc_url( $delete_link ), __( 'Cancel', 'buddypress' ) );
		}
