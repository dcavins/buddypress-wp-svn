...
class BP_Members_Admin {
	...
	public function admin_menus() {
		...
		if ( ( ! bp_is_network_activated() && ! is_network_admin() ) || ( is_network_admin() && bp_is_network_activated() ) ) {
			...
			// Manage opt-outs.
			$hooks['members_optouts'] = $this->members_optouts_page = add_users_page(
				__( 'Manage Opt-outs',  'buddypress' ),
				__( 'Manage Opt-outs',  'buddypress' ),
				$this->capability,
				'bp-members-optouts',
				array( $this, 'optouts_admin' )
			);
		}
	 ...
	 * @since 8.0.0
	 *
	 * @global $bp_members_optouts_list_table
	 */
	public function members_optouts_admin_load() {
		global $bp_members_optouts_list_table;
		...
		/**
		 * Fires at the start of the nonmember optouts admin load.
		 *
		 * @since 8.0.0
		 *
		 * @param string $doaction Current bulk action being processed.
		 * @param array  $_REQUEST Current $_REQUEST global.
		 */
		do_action( 'bp_members_optouts_admin_load', $doaction, $_REQUEST );
		 ...
		 * @param array $value Array of allowed actions to use.
		 */
		$allowed_actions = apply_filters( 'bp_members_optouts_admin_allowed_actions', array( 'do_delete',  'do_resend' ) );

		if ( ! in_array( $doaction, $allowed_actions ) || ( -1 == $doaction ) ) {

			$bp_members_optouts_list_table = self::get_list_table_class( 'BP_Members_Optouts_List_Table', 'users' );

			// The per_page screen option.
			add_screen_option( 'per_page', array( 'label' => _x( 'Nonmember opt-outs', 'Nonmember opt-outs per page (screen options)', 'buddypress' ) ) );

			get_current_screen()->add_help_tab( array(
				'id'      => 'bp-members-optouts-overview',
				'title'   => __( 'Overview', 'buddypress' ),
				'content' =>
				...
				'<p>' . __( 'Using the search form, you can find specific opt-outs more easily. The Email Address field will be included in the search.', 'buddypress' ) . '</p>'
			) );

			get_current_screen()->add_help_tab( array(
				'id'      => 'bp-members-optouts-actions',
				'title'   => __( 'Actions', 'buddypress' ),
				'content' =>
			...
			// Plugins can update other stuff from here.
				...
				/**
				 * Fires at end of member opt-outs admin load
				 ...
				 * @param string $redirect Determined redirect url to send user to.
				 */
				do_action( 'bp_members_admin_update_optouts', $doaction, $_REQUEST, $this->redirect );
	 ...
	 * @return array
	 */
	private function get_members_optouts_notice() {
	...
	/**
	 * Member opt-outs admin page router.
	 *
	 * Depending on the context, display
	 * - the list of optouts,
	 * - or the delete confirmation screen,
	 ...
	 * @since 8.0.0
	 */
	public function optouts_admin() {
		$doaction = bp_admin_list_table_current_bulk_action();

		// Prepare notices for admin.
		$notice = $this->get_members_optouts_notice();
		...
		switch ( $doaction ) {
			case 'delete' :
				$this->optouts_admin_manage( $doaction );
				break;

			default:
				$this->optouts_admin_index();
				break;
		}
	}
	 ...
	 * @global $bp_members_optouts_list_table
	 */
	public function optouts_admin_index() {
		global $plugin_page, $bp_members_optouts_list_table;

		$usersearch = ! empty( $_REQUEST['s'] ) ? stripslashes( $_REQUEST['s'] ) : '';

		// Prepare the group items for display.
		$bp_members_optouts_list_table->prepare_items();
		...
		<div class="wrap">
			...
			<?php $bp_members_optouts_list_table->views(); ?>

			<form id="bp-members-optouts-search-form" action="<?php echo esc_url( $search_form_url ) ;?>">
				<input type="hidden" name="page" value="<?php echo esc_attr( $plugin_page ); ?>" />
				<?php $bp_members_optouts_list_table->search_box( __( 'Search opt-outs', 'buddypress' ), 'bp-members-optouts' ); ?>
			</form>

			<form id="bp-members-optouts-form" action="<?php echo esc_url( $form_url );?>" method="post">
				<?php $bp_members_optouts_list_table->display(); ?>
			</form>
		</div>
	<?php
	}
	 ...
	 * @return null|false
	 */
	public function optouts_admin_manage( $action = '' ) {
		if ( ! current_user_can( $this->capability ) || empty( $action ) ) {
			die( '-1' );
		}
		...
		// These arguments are added to all URLs.
		$url_args = array( 'page' => 'bp-members-optouts' );
