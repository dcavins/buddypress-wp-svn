 ...
 * @since 7.0.0
 */
class BP_Network_Invitation_Manager extends BP_Invitation_Manager {
	/**
	...
	public function run_send_action( BP_Invitation $invitation ) {
		...
		// Notify the invitee of the invitation.
			...
			$unsubscribe_args = array(
				'user_id'           => 0,
				'notification_type' => 'bp-network-invitation',
			);
			$invite_url = esc_url( add_query_arg( array(
				'inv' => $invitation->id,
	...
	public function allow_invitation( $args ) {
		...
		if ( ! bp_user_can( $args['inviter_id'], 'bp_network_send_invitation' ) ) {
			return false;
		}

		// Is the invited user eligible to receive an invitation? Hasn't opted out?
		if ( ! bp_user_can( 0, 'bp_network_receive_invitation', $args ) ) {
			return false;
		}
