 ...
 */
function bp_network_invitations_maybe_skip_activation_email( $send, $user_id, $user_email, $activation_key, $usermeta ) {

	$invite = bp_network_get_invites( array(
...
add_filter( 'bp_core_signup_send_activation_key', 'bp_network_invitations_maybe_skip_activation_email', 10, 5 );
 ...
 */
function bp_network_invitations_complete_signup( $user_id, $user_login, $user_password, $user_email ) {
	if ( ! $user_id ) {
		return;
	}
...
add_action( 'bp_core_signup_user', 'bp_network_invitations_complete_signup', 10, 4 );